...
class BottleNeck_enet(nn.Module):
    ...
    def forward(self, x, indices=None, output_size=None):
        ...
        if self.pad > 0:
            ...
            if torch.cuda.is_available(): extras = extras.cuda()
...
class ENet_EPED(BaseModel):
    ...
    def forward(self, x):
        ...
        # Stage 6
        x = self.fullconv(x)
        # return self.softmax(x)  # 在最后的输出前用softmax函数
